...
class BearPutSpreadHedgeAnalyzer:
                ...
                for date_idx, d_str in enumerate(trading_dates):
                    ...
                    if not in_position:
                        ...
                        if len(strikes) >= 2:
                            ...
                            if atm_idx > 0:
                                K1 = strikes[atm_idx - 1] # Long Put (ATM)
                                K2 = strikes[atm_idx]     # Short Put (OTM lower)
                        ...
                        opt1_today = next((o for o in opts_today if abs(o['strike'] - position['K1']) < 1e-4), None)
                        p1 = opt1_today['close'] if opt1_today else max(0.0, position['K1'] - spot_price)
                        sigma = implied_volatility(p1, spot_price, position['K1'], T_rem, r_rate, option_type='put')

                        d_put1 = bs_delta(spot_price, position['K1'], T_rem, r_rate, sigma, option_type='put')
                        d_put2 = bs_delta(spot_price, position['K2'], T_rem, r_rate, sigma, option_type='put')

                        # Bear Put Spread Delta = Delta(Long Put K1) - Delta(Short Put K2)
                        net_option_delta = (d_put1 - d_put2) * position['qty']
                        ...
                        if position['days_held'] >= position['max_days']:
                            payoff = max(0.0, position['K1'] - spot_price) - max(0.0, position['K2'] - spot_price)
                            opt_pnl = (payoff - position['debit']) * position['qty']
                            fut_pnl = position['fut_pos'] * (spot_price - position['entry_spot'])

                            trade_pnl = opt_pnl + fut_pnl - 10.0 # Fee deduction
...
def generate_reports(instrument_results, steps=[0.05, 0.10, 0.15, 0.20, 0.25, 0.30]):
    ...
    plt.title("Эффективность Захеджированного Bear Put Spread по Инструментам MOEX", fontsize=13)
    plt.xlabel("Инструмент", fontsize=11)
    plt.ylabel("Суммарный PnL (РУБ)", fontsize=11)
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    plt.savefig("bear_put_hedge_chart.png", dpi=300)
    plt.close()
    print("Saved bear_put_hedge_chart.png")

    # Markdown Report Generation
    report = """# Отчет по динамическому хеджированию стратегии Bear Put Spread на MOEX
...
"""
    ...
    with open("bear_put_hedge_report.md", "w", encoding="utf-8") as f:
        f.write(report)
    print("Saved bear_put_hedge_report.md")

if __name__ == "__main__":
    analyzer = BearPutSpreadHedgeAnalyzer()
