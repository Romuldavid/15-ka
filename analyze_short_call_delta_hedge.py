...
class ShortCallDeltaHedgeAnalyzer:
            ...
            for st in steps:
                ...
                for date_idx, d_str in enumerate(trading_dates):
                    ...
                    if not in_position:
                        ...
                        if len(strikes) >= 1:
                            # Find ATM Call strike with delta closest to 0.50 (closest strike to spot_price)
                            closest_idx = min(range(len(strikes)), key=lambda i: abs(strikes[i] - spot_price))
                            K_atm = strikes[closest_idx]

                            opt_call = next((o for o in opts_today if abs(o['strike'] - K_atm) < 1e-4), None)

                            if opt_call and opt_call['close'] > 0:
                                credit_entry = opt_call['close']
                                # Size position bounded by capital and ГО margin requirement
                                qty = min(5, max(1, int((current_capital * 0.05) / (spot_price * 0.15))))

                                # Initial Delta for Short Call is ~ -0.50 per contract
                                in_position = True
                                position = {
                                    'K_atm': K_atm,
                        ...
                        opt_call_today = next((o for o in opts_today if abs(o['strike'] - position['K_atm']) < 1e-4), None)
                        p_call = opt_call_today['close'] if opt_call_today else max(0.0, spot_price - position['K_atm'])
                        sigma = implied_volatility(p_call, spot_price, position['K_atm'], T_rem, r_rate, option_type='call')

                        # Short Call Delta = - Delta(Call) ~ -0.50
                        d_call = bs_delta(spot_price, position['K_atm'], T_rem, r_rate, sigma, option_type='call')
                        net_option_delta = -d_call * position['qty']
                        total_delta = net_option_delta + position['fut_pos']

                        # Rebalance futures hedge if total delta strays by rebalance step
                        if abs(total_delta) > st * position['qty']:
                            position['fut_pos'] = -round(net_option_delta)
                            hedge_actions += 1

                        if position['days_held'] >= position['max_days']:
                            payoff = max(0.0, spot_price - position['K_atm'])
                            # Short Call PnL = Credit Received - Final Payoff
                            opt_pnl = (position['credit_entry'] - payoff) * position['qty']
                            fut_pnl = position['fut_pos'] * (spot_price - position['entry_spot'])

                            trade_pnl = opt_pnl + fut_pnl - 10.0 # Transaction fees
...
def generate_reports(instrument_results, steps=[0.05, 0.10, 0.15, 0.20, 0.25, 0.30]):
    ...
    plt.title("Эффективность Захеджированного Short Call (Дельта ~ 0.50) по Инструментам MOEX", fontsize=13)
    plt.xlabel("Инструмент", fontsize=11)
    plt.ylabel("Суммарный PnL (РУБ)", fontsize=11)
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    plt.savefig("short_call_hedge_chart.png", dpi=300)
    plt.close()
    print("Saved short_call_hedge_chart.png")

    # Markdown Report Generation
    report = """# Отчет по динамическому хеджированию стратегии Short Call (Дельта ~ 0.50) на MOEX
...
"""
    ...
    with open("short_call_hedge_report.md", "w", encoding="utf-8") as f:
        f.write(report)
    print("Saved short_call_hedge_report.md")

if __name__ == "__main__":
    analyzer = ShortCallDeltaHedgeAnalyzer()
